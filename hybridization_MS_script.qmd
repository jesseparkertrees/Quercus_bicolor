...
---
## script for analyses in "EVALUATING THE CENTRAL-MARGINAL HYPOTHESIS: EVIDENCE FOR INCREASED INTROGRESSION AT THE TRAILING EDGE OF QUERCUS BICOLOR"
...
```{r eval=FALSE}
# Perform PCA
pca1 <- glPca(g, center=TRUE, scale=TRUE) # keep 7
...
pca_df1 <- data.frame(
  PC1 = pca1$scores[,1],
  PC2 = pca1$scores[,2],
  Population = as.factor(pop)
)

pca_df1$Population <- factor(pca_df1$Population, levels = c(
  "Primary", "Primary (outlier)", setdiff(unique(pca_df1$Population), c("Primary", "Primary (outlier)"))
))
pca_df1[1,3] <- "Primary (outlier)"
pca_df1$Population2<-1
pca_df1$Population2 <- factor(pca_df1$Population2, levels = c("Primary", "Reference"))
# Now you can assign it
pca_df1[1:135,4] <- "Primary"
pca_df1[136:161,4] <- "Reference"
# Define the color palette
library('viridis')
color_palette1 <- c("purple4","purple4","#00308F", "magenta", "darkgreen", "#FF8000", "#A52A2A","green",  "cyan")
# Select one representative per population for labeling
pca_df1$Population <- as.character(pca_df1$Population)
pca_label1 <- pca_df1 %>%
  group_by(Population) %>%
  dplyr::slice(1)

# Merge to get label positions for each point
pca_df1 <- pca_df1 %>% dplyr::left_join(pca_label1, by = "Population", suffix = c("", "_label"))

# Variance explained (percentages for axis labels)
var_exp1.1 <- round(100 * pca1$eig[1] / sum(pca1$eig), 2)
var_exp2.1 <- round(100 * pca1$eig[2] / sum(pca1$eig), 2)

library(dplyr)
library(cowplot) 
pca_plot1 <- ggplot(pca_df1, aes(x = PC1, y = PC2, color = Population, shape = Population2)) +
  geom_point(size = 4, alpha = 0.8, show.legend = FALSE) +  # Removes color legend
  ...
  geom_text_repel(
    ...
    family = "mono",
    max.overlaps = Inf,
    nudge_x = 0.01 * (max(pca_df1$PC1) - min(pca_df1$PC1)),
    nudge_y = 0.01 * (max(pca_df1$PC2) - min(pca_df1$PC2)),
    force = 2,
    segment.size = 0.7,
    box.padding = 1.2,
    show.legend = FALSE
  ...
  labs(
    x = paste0("PC1 (", var_exp1.1, "% variance)"),
    y = paste0("PC2 (", var_exp2.1, "% variance)")
  )

# Scree plot (eigenvalues barplot)
scree_df1 <- data.frame(PC = 1:length(pca1$eig), Eigenvalue = pca1$eig)
scree_plot1 <- ggplot(scree_df1, aes(x = PC, y = Eigenvalue)) +
...
final_plot1 <- ggdraw() +
  draw_plot(pca_plot1) +
  draw_plot(scree_plot1, x = .1, y = 0.1, width = 0.17, height = 0.25)
final_plot1


# Create second plot
vc2<-read.vcfR("./data/STACKS/dataset2/populations.snps.vcf")
g<-vcfR2genlight(vc2)
g<-gl.compliance.check(g)
names<-g@ind.names
pops<-read.csv("./data/popmap_dataset2_version2.csv", header=TRUE)
pop<-pops$pop %>% as.factor()
g@pop<-pop
g<-gl.compliance.check(g)
#for just Qlyrata and Qbicolor, filter others out
#glv<-gl.drop.ind(x=glv,ind.list=c("sample_1","SRR5284353_quma_NM","SRR5284354_quma_QUEBEC","SRR5284355_quma_IL","SRR5284356_quma_MO","SRR5632360_quma_ND","SRR5632409_quma_KS","SRR5284372_qual_MI", "SRR5284374_qual_NC","SRR5284375_qual_MN","SRR5284342_qumo_NC","SRR5284343_qumo_VA","SRR5284344_qumo_VA","SRR5632531_qumo_NC"  ,"SRR5284339_qumu_TX","SRR5284340_qumu_OK","SRR5284341_qumu_IL","SRR5632461_qumu_IL","SRR5632513_qust_NC","SRR5632603_qust_AL"), mono.rm=TRUE, recalc=TRUE)
g@ind.names
names<-g@ind.names
# Perform PCA
...
# Define the color palette
color_palette<-c("purple","#4777EFFF" ,"#4686FBFF","#3BA0FDFF", "#28BBECFF", "#1AD3D1FF","#1AE4B6FF", "#31F299FF", "#56FA75FF", "#80FF53FF","#A2FC3CFF", "#BEF434FF", "#D9E436FF", "magenta",  "darkgreen")

# Generates 28 colors
# Select one representative per population for labeling
pca_label <- pca_df %>%
  group_by(Population) %>%
  dplyr::slice(2)  # Take the first occurrence per population

# Merge to get label positions for each point
pca_df <- pca_df %>% left_join(pca_label, by = "Population", suffix = c("", "_label"))
levels(pca_df[,3]) <- c(levels(pca_df[,3]), "Ref. Q. bicolor", "Ref. Q. lyrata")
pca_df[135:137,3]<-"Ref. Q. bicolor"
pca_df[138:141,3]<-"Ref. Q. lyrata"


pca_df$Population2 <- 1
levels(pca_df[,6]) <- c("Primary", "Reference")
# Now you can assign it
pca_df[1:134,6] <- "Primary"
pca_df[135:141,6] <- "Reference"
...
pca_plot <- ggplot(pca_df, aes(x = PC1, y = PC2, color = Population, shape=Population2)) +
  ...
  guides(color = guide_legend(override.aes = list(shape = c(rep(16, 13), 17, 17))),shape = guide_legend(override.aes = list(color = "black")))+
  labs(
...
# Combine final_plot1 and final_plot2
library(patchwork)
library(cowplot)
library(ggplot2)
# Use theme text with monospace font for labels
pca_plot_labeled <- ggdraw(final_plot1) +
  draw_plot_label("A", x = 0, y = 1, hjust = -0.5, vjust = 1.2, 
                  fontface = "bold", size = 28, family = "mono")

scree_plot_labeled <- ggdraw(final_plot2) +
  draw_plot_label("B", x = 0, y = 1, hjust = -0.5, vjust = 1.2, 
                  fontface = "bold", size = 28, family = "mono")

# Stack vertically with small gap (plot_spacer adds space)
combined <- pca_plot_labeled / plot_spacer() / scree_plot_labeled + 
  plot_layout(heights = c(1, 0.05, 1))  # small gap between
# Show plot
combined
```
...
# Geospatial analysis
## Maxent models for Quercus bicolor
```{r eval=FALSE}
library(dplyr)
library(terra)
library(SDMtune)
...
output_folder <-"~/Quercus_bicolor/geospatial/env_data"
...
```
## Model hybrid zones using paleoclimate data
```{r eval=FALSE}
# get paleo data for the last glacial maximum from https://www.worldclim.org/data/v1.4/paleo1.4.html
# put in "~/Quercus_bicolor/geospatial/env_data/lgm"
output_folder <-"~/Quercus_bicolor/geospatial/env_data/lgm"
files <- list.files(output_folder, pattern = "\\.tif$", full.names = TRUE)
nontemp<-rast(files)
#convert temperature to standard units
pal<-temp/10
paleo<-c(pal,nontemp)
...
# get midholocene data from https://www.worldclim.org/data/v1.4/paleo1.4.html
# put in "~/Quercus_bicolor/geospatial/env_data/midhol"
output_folder <-"~/Quercus_bicolor/geospatial/env_data/midhol"
...
# get future climate from https://www.worldclim.org/data/cmip6/cmip6_clim2.5m.html (EC-Earth3-Veg)
# put in "~/Quercus_bicolor/geospatial/env_data/fut"
fut<-rast("~/Quercus_bicolor/geospatial/env_data/fut/wc2.1_2.5m_bioc_EC-Earth3-Veg_ssp585_2041-2060.tif")
...
library(sf)
library(geodata)
color_mapping <- c("0" = NULL, "1" =  "#FDE725FF", "2" = "#440154FF", "3" = "#21908CFF" )
...
GeomRaster$draw_key <- ggplot2::draw_key_polygon
...
ig_overlap_map<-ggplot() +
  ...
  scale_fill_manual(na.value = "transparent",
    ...
    name="",guide = guide_legend(
      override.aes = list(
        colour = "black",   # border color (spelled "colour")
        fill = color_mapping,  # needed for draw_key_polygon()
        shape = NA)) ,
    labels=c(expression(italic("Q. bicolor  ")), 
             expression(italic("Q. lyrata  ")), 
...
midhol_overlap<-crop(midhol_overlap, dissolve_sf)
...
current_overlap<-crop(current_overlap, dissolve_sf)
...
fut_overlap<-crop(fut_overlap, dissolve_sf)
...
# Shared theme to standardize spacing and margins
uniform_theme <- theme(plot.margin = margin(10, 10, 10, 10),
        legend.position = "bottom",
...
lgm_overlap_map <- lgm_overlap_map + labs(title = NULL) + uniform_theme
midhol_overlap_map <- midhol_overlap_map + labs(title = NULL) + uniform_theme
current_overlap_map <- current_overlap_map + labs(title = NULL) + uniform_theme
fut_overlap_map <- fut_overlap_map + labs(title = NULL) + uniform_theme

# Combine plots with shared legend
combined <- (lgm_overlap_map + midhol_overlap_map) /
  (current_overlap_map + fut_overlap_map) +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom", legend.title=element_text(size=24), legend.text=element_text(size=18),plot.margin = margin(7, 7, 0, 7, unit='mm'))  # spacing between plots

# Convert to grob using patchwork internal function
combined_grob <- patchwork:::patchworkGrob(combined)

# Add panel labels with cowplot
final_plot <- ggdraw(combined_grob) +
  draw_plot_label(
    label = c("A", "B", "C", "D"),
    x = c(0.0, 0.48, 0.0, 0.48),   # X positions
    y = c(1, 1, 0.51, 0.51),     # Y positions
    size = 36,
    fontface = "bold",
    family = "mono"
  )
...
print(final_plot)
```
# Plot range maps of sympatric oaks
```{r eval=FALSE}
library(terra)
library(geodata)
library(sf)
# get range maps for species from https://github.com/wpetry/USTreeAtlas
# put in "~/Quercus_bicolor/geospatial/env_data/rangemaps"

quma<-vect("~/Quercus_bicolor/geospatial/env_data/rangemaps/quermacr.shp")
qual<-vect("~/Quercus_bicolor/geospatial/env_data/rangemaps/queralba.shp")
qumu<-vect("~/Quercus_bicolor/geospatial/env_data/rangemaps/quermueh.shp")
qust<-vect("~/Quercus_bicolor/geospatial/env_data/rangemaps/querstel.shp")
crs(qust)<-crs(quma)
qumo<-vect("~/Quercus_bicolor/geospatial/env_data/rangemaps/querprin.shp")
qubi<-vect("~/Quercus_bicolor/geospatial/env_data/rangemaps/querbico.shp")
qumi<-vect("~/Quercus_bicolor/geospatial/env_data/rangemaps/quermich.shp")
quly<-vect("~/Quercus_bicolor/geospatial/env_data/rangemaps/querlyra.shp")
...
qubi_map<-ggplot() +
  geom_sf(data = qubi_sf, fill = "darkseagreen", color = 'black') +
  
  # US borders
  geom_sf(data=us_sf_clipped, fill=NA, color="black", size=0.5) +
  coord_sf(xlim=c(min(df$x), max(df$x)), ylim=c(min(df$y), max(df$y))) +
  labs(x="Longitude", y="Latitude") +
  labs(title="Q. bicolor") +
  theme_minimal()+
  theme(
    text = element_text(family="mono"),             # Set all text to monospace
    title = element_text(family="mono", face="italic"),            # Monospace for title
    axis.title = element_text(family="mono"),       # Monospace for axis labels
    axis.text = element_text(family="mono"),        # Monospace for axis ticks
    legend.text = element_text(family="mono"), 
    legend.title = element_text(family="mono")      # Monospace for legend title
...
quma_sf <- st_as_sf(quma)
quma_map<-ggplot() +
  geom_sf(data = quma_sf, fill = "darkseagreen", color = 'black') +
...
qumi_sf <- st_as_sf(qumi)
qumi_map<-ggplot() +
  geom_sf(data = qumi_sf, fill = "darkseagreen", color = 'black') +
  
  # US borders
  geom_sf(data=us_sf_clipped, fill=NA, color="black", size=0.5) +
  coord_sf(xlim=c(min(df$x), max(df$x)), ylim=c(min(df$y), max(df$y))) +
  labs(x="Longitude", y="Latitude") +
  labs(title="Q. michauxii") +
  theme_minimal()+
  theme(
    text = element_text(family="mono"),             # Set all text to monospace
    title = element_text(family="mono"),            # Monospace for title
...
quly_map<-ggplot() +
  geom_sf(data = quly_sf, fill = "darkseagreen", color = 'black') +
...
qual_sf <- st_as_sf(qual)
qual_map<-ggplot() +
  geom_sf(data = qual_sf, fill = "darkseagreen", color = 'black') +
...
qust_sf <- st_as_sf(qust)
qust_map<-ggplot() +
  geom_sf(data = qust_sf, fill = "darkseagreen", color = 'black') +
...
qumu_sf <- st_as_sf(qumu)
qumu_map<-ggplot() +
  geom_sf(data = qumu_sf, fill = "darkseagreen", color = 'black') +
...
qumo_sf <- st_as_sf(qumo)
qumo_map<-ggplot() +
  geom_sf(data = qumo_sf, fill = "darkseagreen", color = 'black') +
  
  # US borders
  geom_sf(data=us_sf_clipped, fill=NA, color="black", size=0.5) +
  coord_sf(xlim=c(min(df$x), max(df$x)), ylim=c(min(df$y), max(df$y))) +
  labs(x="Longitude", y="Latitude") +
  labs(title="Q. montana") +
  theme_minimal()+
  theme(
    text = element_text(family="mono"),             # Set all text to monospace
    title = element_text(family="mono"),            # Monospace for title
...
# Shared theme to standardize spacing and margins
uniform_theme <- theme(
  plot.margin = margin(10, 10, 10, 10),  # increased margin for spacing
...
# Apply theme to all maps
#qual_map <- qual_map + labs(title = NULL) + uniform_theme
qual_map <- qual_map + labs(title = "Q. alba") +uniform_theme
qubi_map <- qubi_map + labs(title = "Q. bicolor") +uniform_theme
quma_map <- quma_map + labs(title = "Q. macrocarpa") +uniform_theme
qumi_map <- qumi_map + labs(title = "Q. michauxii") +uniform_theme
quly_map <- quly_map + labs(title = "Q. lyrata") +uniform_theme
qust_map <- qust_map + labs(title = "Q. stellata") +uniform_theme
qumu_map <- qumu_map + labs(title = "Q. muehlenbergii") +uniform_theme
qumo_map <- qumo_map + labs(title = "Q. montana") +uniform_theme
# Combine plots with shared legend
combined <- (
  qubi_map + quly_map + quma_map + qumi_map +
    qual_map + qumo_map + qumu_map + qust_map
) +
  ...
  theme(
    ...
    plot.margin = margin(7, 7, 7, 7, unit = 'mm')
  )


final_plot <- ggdraw(patchwork:::patchworkGrob(combined))

# Print final plot
print(final_plot)

# Save the final plot
ggsave("figure1.pdf", plot = final_plot, width = 16, height = 8, dpi = 800)
ggsave("figure1.png", plot = final_plot, width = 16, height = 8, dpi = 800)
```
